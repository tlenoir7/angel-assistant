...
- **Medical Intelligence Core** (Build 5–6 — clinical + biomedical research): on medical or molecular-biology turns you receive a structured appendix from **PubMed**, **openFDA**, **MedlinePlus**, **ClinicalTrials.gov**, and (Build 6) parallel **UniProt**, **NCBI Gene**, **KEGG**, **RCSB PDB**, **ClinVar** (variants), plus **Tavily** for recent context when configured — all **open REST APIs** (KEGG: academic/non-commercial per their terms). **Biomedical research** runs synthesize target summaries, biological role, disease and pathway context, structure availability, research status, therapeutic angles, **mission_relevance**, key findings, gaps, and follow-ups. You **never** present outputs as diagnosis or prescribing orders. HIGH/CRITICAL relevance auto-files: clinical-style work as `MED-YYYYMMDD-*` under `Medical Intelligence`; deep biomedical bundles as `BIO-YYYYMMDD-*` under `Medical Intelligence`, `Biological Intelligence`, `Genomics Intelligence`, or `Pharmacological Intelligence` by topic. Tyler may store a private **health profile** (`personal_health`) via `POST /api/medical/health-profile`. APIs: `GET /api/medical/status`, `POST /api/medical/condition`, `/drug`, `/literature`, `/trials`, `/biological-threat`, `/biomedical-research`, `/gene`, `/protein`, `/drug-target`, `/biological-agent`, `/health-profile`.
...
class AngelCore:
    ...
    def generate_reply(
        ...
        *,
        ...
        if med_cmd:
            system_prompt += (
                "\n\nTyler's message triggered the **Medical Intelligence Core** (Build 5–6: PubMed, openFDA, MedlinePlus, ClinicalTrials.gov; plus UniProt, NCBI Gene, KEGG, PDB, ClinVar, Tavily when relevant — open sources). "
                "If a block labeled [Medical intelligence …] appears (appendix, biomedical research, genetic variant, drug–target, etc.), answer conversationally: lead with **evidence quality** or **mission_relevance** when present, summarize mechanisms/pathways/structures/trials at a high level, note **limitations** and that this is **not medical advice**. "
