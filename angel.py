...
- **Medical Intelligence Core** (Build 5–8 — clinical, biomedical, theoretical treatment design, **personal health intelligence**): on medical, molecular-biology, or **treatment-design** turns you receive structured appendices from **PubMed**, **openFDA**, **MedlinePlus**, **ClinicalTrials.gov**, **UniProt**, **NCBI Gene**, **KEGG**, **RCSB PDB**, **ClinVar**, and **Tavily** when configured (KEGG: academic/non-commercial per their terms). Build 7 adds **theoretical treatment design** — novel *investigative* combinations of mechanisms, compounds, and pathways when Tyler asks to design, optimize combinations, survey repurposing or exotic modalities, or map **UAP-adjacent** symptom reports to open literature on radiation, EM fields, acoustic/infrasound effects, and analogous protocols. **Always label Build 7 outputs as THEORETICAL** and include verbatim: *"This is a theoretical research synthesis for educational and investigative purposes only. It does not constitute medical advice. Any treatment decisions must be made with qualified medical professionals."* **Build 8** loads Tyler's **private** structured health profile from Mem0 (`category: personal_health`) to personalize reasoning—never recite the full profile unprompted; never put it in memory summaries, briefings, digests, or exports; use it only to tailor answers and flag plausible med/allergy conflicts (open sources; not prescribing). You never prescribe or replace licensed care. Auto-file **MEDIUM+** mission relevance: `RX-YYYYMMDD-*` under `Theoretical Medicine`, `Medical Intelligence` (repurposing), or `UAP Medical Intelligence` (witness-health cross-references); retain `MED-*` / `BIO-*` rules from prior builds. Profile & APIs: `POST /api/medical/health-profile` (or `updates` in body), `GET /api/health/profile`, `POST /api/health/update`, `/api/health/assess`, `/interactions`, `/recommendations`, `/wearable`, `GET /api/health/status`. Also: `GET /api/medical/status`, `POST /api/medical/condition`, `/drug`, `/literature`, `/trials`, `/biological-threat`, `/biomedical-research`, `/gene`, `/protein`, `/drug-target`, `/biological-agent`, `/design-treatment`, `/optimize-combination`, `/repurposing`, `/exotic-treatments`, `/uap-medical`.
...
class AngelCore:
    ...
    def __init__(self, user_id: str, use_voice: bool = False, allow_computer_control: bool = False):
        ...
        try:
            import angel_medical as _amed_init

            _amed_init.get_health_profile(
        ...
        *,
        ...
        def _bg_health_extract() -> None:
            try:
                ...
                import angel_medical as _amed

                ex = _amed.extract_health_info_from_conversation(
                    user_message,
                    anthropic_client=self.anthropic_client,
                )
                log = logging.getLogger("angel.health")
                if ex.get("urgent"):
                    log.warning("Urgent health mention: %s", ex.get("urgent_reason") or "")
                if str(ex.get("confidence") or "").upper() == "HIGH" and isinstance(ex.get("extracted"), dict) and ex["extracted"]:
                    r = _amed.update_health_profile(
                        ex["extracted"],
                        memory_client=self.memory_client,
                        user_id=self.user_id,
                        use_mem0_cloud=self._use_mem0_cloud,
                    ...
                    if r.get("ok"):
                        log.info("Health profile updated: %s", list(ex["extracted"].keys()))

            except Exception:
                pass

        threading.Thread(target=_bg_health_extract, daemon=True).start()
        ...
        if re.search(
            r"(?i)\b(chest pain|crushing chest|can't breathe|cannot breathe|shortness of breath|"
            r"suicidal|kill myself|heart attack|stroke symptoms|severe allergic reaction|anaphylaxis)\b",
            user_message or "",
        ):
            system_prompt += (
                "\n\n[URGENT safety: Tyler's message may describe serious symptoms or crisis language. "
                "Prioritize emergency medical guidance (call local emergency services when appropriate) and crisis resources; "
                "do not diagnose; encourage immediate in-person care when warranted.]"
            )
        ...
        if med_cmd:
            system_prompt += (
                "\n\nTyler's message triggered the **Medical Intelligence Core** (Build 5–8: clinical DBs, biomedical parallel research, theoretical treatment design, and **personal health context** when relevant — open sources). "
                "If a block labeled [Medical intelligence …] appears (appendix, biomedical, genetic, drug–target, **THEORETICAL treatment design**, combination optimization, repurposing, exotic modalities, UAP medical cross-reference), answer conversationally. "
                "For **Build 7 / theoretical** blocks, state clearly that the approach is **hypothetical investigative synthesis**, lead with **feasibility** and **evidence_base** when present, and **always** repeat the full disclaimer: "
                "This is a theoretical research synthesis for educational and investigative purposes only. It does not constitute medical advice. Any treatment decisions must be made with qualified medical professionals. "
                "For other medical blocks: lead with **evidence quality** or **mission_relevance**, note limitations, not medical advice. Offer deeper dives on subsections. Flag black-box warnings when present."
            )
        if med_cmd or bio_cmd:
            try:
                import angel_medical as _amed

                hc = _amed.format_health_context_for_medical_prompt(
                    memory_client=self.memory_client,
                    user_id=self.user_id,
                    use_mem0_cloud=self._use_mem0_cloud,
                )
